...
int main() {
    ...
    InstrumnetGainer* ig = new InstrumnetGainer(marketAddress, brokerID, investorID, password);

    while(!ig->IsLogin()){
        printf("InstrumnetGainer Login ...\n");
        sleep(1);
    }
    ...
    while(!ig->IsEnd()){
        sleep(1);
        printf("InstrumnetGainer ReqAllInstrument ...\n");
    }
    int r_len = ig->Len();
    char **res = ig->GetRes();
